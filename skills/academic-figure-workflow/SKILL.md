...
## Visual inspection authorization

Rendering and deterministic geometry, glyph-coverage and package checks may run within the authorized artifact task. Model-based image viewing or manual pixel inspection requires the user's explicit authorization when global rules prohibit proactive vision. This condition applies to every inspection step in this skill and its references. Without that authorization, complete the permitted checks and report visual acceptance as unverified; do not claim full visual QA or full figure completion. Request authorization only when visual acceptance is required to finish the user's task.
...
7. **Rendered gate**: measure after `document.fonts.ready`, export a 600 dpi PNG, and inspect the actual pixels. For mixed Chinese/Latin text, use the exact final renderer to produce a text-only diagnostic and require every non-empty SVG `<text>` element to yield raster ink; installed-font checks alone do not prove glyphs were drawn. Repeat until automatic issues are zero.
...
When visual inspection is authorized, inspect the final-size PNG at 100% and zoomed views. Verify:
...
- no Chinese/Latin text run is missing in the final PNG; compare the editable source's non-empty text objects against a text-only raster produced by the same export engine;
