...
| SVG/browser preview contains Chinese but the PNG embedded in Word drops whole Chinese runs | The rasterizer selects one Latin face for a mixed-script run and does not perform per-glyph CJK fallback; checking installed fonts or source text alone gives a false pass | Export with a renderer that performs per-glyph fallback (browser/Pango), or split mixed-script runs into explicit script-specific `<tspan>` elements; rebuild from editable source rather than patching the broken PNG | Render a text-only diagnostic through the exact final export path and require every non-empty `<text>` element to produce raster ink; then inspect the actual embedded PNG and rendered DOCX page |
...
| A formula is visually typed as plain prose (`ECTc`, `Cprefill`, `i*`, Unicode pseudo-subscripts) | Mathematical notation was entered as one italic text run instead of being typeset by semantic role | Use upright operators/functions, italic variables, explicit superscript/subscript baselines, operator limits, and aligned continuation lines; keep symbol fallback explicit | Inspect the final raster for real baselines and consistent formula size; notation audit confirms no prose-style fake superscripts/subscripts or all-italic operators |
