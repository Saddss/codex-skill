...
description: >
  Profiles and optimizes LLM inference host / CPU overhead (vLLM / SGLang /
  TRT-LLM serve / lmdeploy / TensorRT-LLM PyExecutor) using line_profiler (with
  nsys support planned). Runs iterative profile-analyze-optimize-validate
  rounds. Use when GPU utilization is low, when perf-host-analysis has
  concluded "host-bound", or when the user asks to reduce host overhead /
  Python scheduling overhead / sampler latency / inference loop overhead.
  Triggers also include: "削 host overhead", "降 launch overhead", "优化
  scheduler", "GPU 利用率低怎么提".
license: Apache-2.0
metadata:
  tags: [optimization, profiling, host-overhead, line-profiler, inference]
  author: NVIDIA Corporation
  upstream: https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/perf-host-optimization
  adapted_for: Codex global skills (inference-server triggers added)
---

> **Vendored from [NVIDIA/TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/perf-host-optimization) under Apache-2.0.** Only the `description` was extended with broader inference-server triggers; the body and references are verbatim. NVIDIA's copyright + license header is preserved in this fork's `LICENSE-Apache-2.0.txt`.
>
> **`references/hot-path-files.md` is TRT-LLM-specific** — it lists the most-edited Python files in TensorRT-LLM's serving stack. The methodology (find the top-K Python files responsible for inference-loop time, optimize them one at a time, validate via re-profile) carries over to vLLM / SGLang verbatim, but the file paths inside will not match those engines. Substitute the equivalents (e.g. vLLM's `vllm/engine/*.py`, `vllm/core/scheduler.py`) when applying this skill there.
