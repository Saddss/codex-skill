...
description: >
  OpenAI Triton (@triton.jit) kernel development for inference/training ops.
  NEVER use for CUDA C++, TileIR, or profiling tools (ncu, nsys). Covers
  fused elementwise, reductions (softmax, LayerNorm, RMSNorm), tiled GEMM
  with triton.autotune, and flash attention. Workflow: design, write, verify.
  Use when implementing custom attention, MoE, or KV-cache kernels in vLLM /
  SGLang stacks. Triggers include "write triton kernel", "Triton GEMM".
license: Apache-2.0
metadata:
  tags: [triton]
  author: NVIDIA Corporation
  upstream: https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/kernel-triton-writing
  adapted_for: Codex global skills (inference triggers added)
---

> **Vendored from [NVIDIA/TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/kernel-triton-writing) under Apache-2.0.** Includes `scripts/verify_kernel.py` and references. Pair with `perf-nsight-compute-analysis` for optimization. See `LICENSE-Apache-2.0.txt`.
