...
description: >-
  ...
  time from timeline traces, profile distributed training with
  NCCL overlap analysis, or profile an LLM inference server
  (vLLM / SGLang / TRT-LLM serve / lmdeploy) and its client
  (e.g. online_replay.py) end-to-end. Triggers also include
  "用 nsys 抓 profile" / "看 GPU 空闲" / "排查 e2e 延迟" / "分析 NCCL overlap"
  / "trace 推理服务". NOT for kernel-level metrics like SOL%,
...
metadata:
  ...
  upstream: https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/perf-nsight-systems
  adapted_for: Codex global skills (inference-server triggers added)
---

> **Vendored from [NVIDIA/TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/perf-nsight-systems) under Apache-2.0.** Only the `description` was extended with inference-server (vLLM / SGLang / TRT-LLM serve) triggers; the body is verbatim. The NVIDIA copyright + license header is preserved in this fork's `LICENSE-Apache-2.0.txt`.
>
> **In this fork there is no `perf-profiling-specialist` agent** (NVIDIA ships one alongside this skill in the TRT-LLM repo). The workflows below describe running `nsys` directly. For end-to-end inference QPS binary-search work, this skill composes with `model-perf-binary-search` (see that skill's "Profiling integration" section for how to hand off between the two).
