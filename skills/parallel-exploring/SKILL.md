...
description: >-
  Explore a large codebase in parallel by launching multiple explore subagents
  that each investigate a different area simultaneously. Use when onboarding
  onto a new project, understanding architecture, investigating cross-cutting
  concerns, or mapping vLLM / SGLang / TRT-LLM / FlexKV code paths.
metadata:
  upstream: https://github.com/spencerpauly/awesome-cursor-skills/tree/main/resources/parallel-exploring
  adapted_for: Codex global skills (inference triggers added)
---

> **Vendored from [spencerpauly/awesome-cursor-skills](https://github.com/spencerpauly/awesome-cursor-skills).** Parallel codebase exploration workflow.
...
An exploration subagent is a fast, read-only agent optimized for searching and reading code. You can launch multiple exploration agents concurrently, each investigating a different area.
...
2. **Launch parallel explore agents** — use the available subagent mechanism for each area and launch them concurrently:
