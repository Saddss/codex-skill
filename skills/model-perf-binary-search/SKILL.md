...
eval "$(bash ~/.codex/skills/model-perf-binary-search/scripts/bootstrap.sh)"
...
"$LLM_BENCH_DIR/.venv/bin/python" \
  ~/.codex/skills/model-perf-binary-search/scripts/health_check.py --workdir "$LLM_BENCH_DIR"
...
**Step 6 - Service swap for the second session.** Obtain explicit authorization covering the identified service, approved configurations, permitted stop/relaunch operations and any force-stop escalation. One authorization may cover the specified sequence and its task-created replacement services. Before each swap, verify the recorded PID/container identity, start time, ownership and resource allocation; proceed without another question when the operation remains covered. Ask again if the target, configuration, resource scope or impact changes. Send SIGTERM only to the verified target; after a 30-second grace period, reverify identity and use SIGKILL only if authorized. Keep unrelated services running. If stopping is not authorized, report that limitation and do not swap.
...
5. **Run each approved configuration as its own full binary search**, using the bounded service-swap authorization from Step 6 of Mode A. An approved experiment sequence may cover its identified service and task-created replacements; recheck identity before every swap and ask again only for operations outside that authorization. Honour the same progress-monitoring cadence (15 / 30 min) across all stages — the wall-clock timer is per **session**, not per configuration.
...
The agent manages the authorized service startup and leaves the service running by default. Stopping or switching it requires the bounded authorization described in Mode A Step 6:

- Start the service **once** at the beginning by running the user-provided command in the background. Capture its PID, start time, command, resource allocation and stdout/stderr log path. Record replacement identities after each authorized service swap.
...
- **When the binary search finishes, leave the service running unless its shutdown was explicitly authorized.** Client cleanup must be covered by the current task's authorization and target only the recorded, reverified probe PIDs or dedicated group. If ownership cannot be verified, report the remaining processes and request direction. Never terminate processes by matching a command name. Report the observed service state, its identity and log path.
...
When the user's startup command starts with `docker run …` (vLLM/SGLang/TRT-LLM official images, custom containers), use the recorded container ID for lifecycle operations. Capture the ID returned by the launch and its creation/start time; record a new identity after each authorized replacement. Required tweaks:
...
- **Capture logs.** Follow logs using the recorded container ID, save them under `bench-runs/service_<ts>.log`, and record the log follower's PID and start time for task-scoped cleanup. Report the log path in the final report.
- **Liveness check.** Verify the recorded identity and inspect that container using `docker inspect --format '{{.State.Running}}' <recorded_container_id>`. Read state, exit details and logs by the same ID. Distinguish a stopped or removed container from lookup/daemon errors; partial name matches cannot establish liveness. For an automatically removed container, use the captured logs and report unavailable exit details.
- **Service swap (Mode A/B).** Apply the same bounded authorization and identity checks to the recorded container ID. Use the authorized stop timeout and escalation policy; Docker's eventual force-stop must be covered by that authorization. Explain that `--rm` deletes the task-created container after it stops. Preserve mounted data and unrelated containers.
- **Cleanup-on-finish policy is identical**: leave the container running unless its shutdown was explicitly authorized. Report its recorded ID, observed state and log path.
...
5. Run **one** `online_replay.py` process for `q <= 10`. Hash-sampled
   ...
   Record every launched shard's PID, start time and command in the current probe's artifacts. If using a process group, create and record a dedicated group containing only this probe's processes; never identify owned processes by command-name matching alone.
6. Wait for **all** shards to exit and complete the required 8/16 rounds, except for the explicitly authorized queue-runaway exception in Interpretation rules.
...
```bash
python3 ~/.codex/skills/model-perf-binary-search/scripts/analyze_rounds.py \
...
```bash
# before the probe
python3 ~/.codex/skills/model-perf-binary-search/scripts/prefix_cache_hit_rate.py snapshot \
...
python3 ~/.codex/skills/model-perf-binary-search/scripts/prefix_cache_hit_rate.py diff \
...
- All probes must run to completion except for an explicitly authorized **obvious-FAIL queue-runaway** policy covering the current probe and the conditions below. If that policy is authorized, reverify each recorded shard's PID, start time and command, then terminate only those verified processes or the verified dedicated probe group. Never use `pkill -f` or terminate unrelated replay tasks. Write `Result=FAIL`, `hit_rate=n/a`, preserve the observed evidence, and proceed with the bisect when **either**:
  ...
  Without the required authorization or verified process identity, report the runaway and request direction before terminating anything. Document any early-stopped probe and the observed condition in the final report; do not present it as a completed latency measurement.
...
- the recorded service identity, stdout/stderr log path and observed running/stopped state, including any authorized shutdown that occurred.
