...
description: >
  Benchmark ingress-layer hot/cold pool separation for LLM serving against a
  unified cache-aware baseline. Use for ingress split benchmarks, hot/cold GPU
  ratio sweeps, or real vLLM pool validation. The workflow profiles a replay
  dataset, measures baseline QPS under an SLO, sweeps ratios, and reports the
  best ratio with latency, cache, and imbalance metrics. It includes
  policy-fidelity and orchestration-fidelity tiers plus fixes for container
  termination, failing ratio handling, CDI GPU passthrough, and glibc-vs-musl
  image issues.
---
