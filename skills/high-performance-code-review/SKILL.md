...
Inspect these dimensions as needed by the diff; they do not prescribe the response order:

1. **Technical context** — Understand the changed behavior and relevant mechanisms, such as tensor shapes, memory layout, synchronization, CUDA behavior, distributed communication, attention, KV cache, or tensor parallelism. Assess whether the change is necessary, duplicates existing behavior, or adds unjustified complexity. Include only the context needed to explain a finding.
...
List actionable findings ordered by severity and then file/line, with necessary context attached to the relevant finding. Include supported goal-completeness and verification gaps. If there are no actionable findings, say so explicitly and mention residual risks or necessary tests not run. A review-only request does not authorize code changes. Comment language and content follow the user's global rules, including necessary interface, unit and boundary documentation.
