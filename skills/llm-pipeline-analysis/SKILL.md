...
description: >-
  Layer-level torch profiler trace analysis: forward-pass boundaries, anchor
  kernels, per-layer timing clusters, Perfetto navigation. Use when
  llm-torch-profiler-analysis three-table summary is too coarse and you need
  which layers or prefill/decode forward passes dominate.
metadata:
  author: BBuf
  upstream: https://github.com/BBuf/AI-Infra-Auto-Driven-SKILLS/tree/main/skills/llm-pipeline-analysis
  adapted_for: Codex global skills (vendored)
---

> **Vendored from [BBuf/AI-Infra-Auto-Driven-SKILLS](https://github.com/BBuf/AI-Infra-Auto-Driven-SKILLS).** Drill-down companion to `llm-torch-profiler-analysis`.
...
python3 scripts/layer_kernel_breakdown.py \
  --trace $TRACE --config $CONFIG \
  --fwd-pass 5 --layer 3 --format json > work/layer3_detail.json
```
