...
description: >
  ...
  Use when applying optimizations, writing kernels, or improving vLLM /
  SGLang / TRT-LLM / lmdeploy performance. Covers user-specified and
  autopilot-driven iterative optimization.
license: Apache-2.0
metadata:
  tags: [optimization, specialist-routing, kernel-generation]
  author: NVIDIA Corporation
  upstream: https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/perf-optimization
  adapted_for: Codex global skills (capability-based execution; scoped verification)
---

> **Vendored from [NVIDIA/TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/perf-optimization) under Apache-2.0.** Routes to `kernel-triton-writing`, `kernel-cute-writing`, `perf-nsight-compute-analysis`, and other perf-* skills in this repo. See `LICENSE-Apache-2.0.txt`.

# Performance Optimization Coordination

## Specialists

The specialist names below identify task responsibilities. Use an available, authorized subagent when it helps; otherwise perform that responsibility directly using the relevant skill and tools. Routing examples using “delegate” follow this rule and do not require a named agent to exist.
...
- Read the applicable skill before implementing or delegating its workflow.
- Choose direct execution or available, authorized delegation based on the task and environment.
- Include correctness checks and benchmarking in the implementation scope.
- Keep resource ownership, user constraints and output locations explicit when delegating.
- Missing specialist agents do not block direct work when the relevant technical instructions and tools are available. Report genuinely missing capabilities without substituting a different requested technology.
...
The executor of each change owns its correctness checks. Continue safe, authorized implementation and measurement until the requested completion criteria are met or a concrete external blocker remains.
...
1. Preserve the current user changes and an appropriate recoverable baseline before editing.
2. Apply scoped changes with the available file-editing tool, directly or through an authorized subagent.
3. Run relevant accuracy checks and comparable before/after measurements.
4. Keep validated improvements. If restoration is needed, reread current files and use explicit file patches to restore only task-owned changes from the baseline. Never restore code with Git commands. Confirm scope when user edits overlap.
...
Use the measurements needed by the user: affected files, comparable before/after metrics, correctness evidence and remaining limitations. Put necessary technical context beside its finding. The templates below are optional data layouts; follow the user's requested presentation and omit summary sections when prohibited.
