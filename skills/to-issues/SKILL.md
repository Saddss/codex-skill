...
metadata:
  ...
  adapted_for: Codex global skills (inference triggers; setup skill name normalized)
---
...
- For infra: a slice might be "one routing policy + real local server integration test + metrics hook" — end-to-end within that policy
...
| Before | After |
|--------|-------|
| Implementation plan, `grill-with-docs`, approved spec | Issues on tracker |
| Issues published | Ready for implementation when authorized |
| PR ready | `review` |
