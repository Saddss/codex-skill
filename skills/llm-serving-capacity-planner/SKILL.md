...
description: >-
  Parse SGLang/vLLM startup logs for GPU memory decomposition (weights, KV pool,
  CUDA graph, overhead) and max-concurrency estimates. Use for KV cache budget,
  mem-fraction-static tuning, OOM triage, or explaining why concurrency is capped.
metadata:
  author: BBuf
  upstream: https://github.com/BBuf/AI-Infra-Auto-Driven-SKILLS/tree/main/skills/llm-serving-capacity-planner
  adapted_for: Codex global skills (paths normalized to skill-relative)
---

> **Vendored from [BBuf/AI-Infra-Auto-Driven-SKILLS](https://github.com/BBuf/AI-Infra-Auto-Driven-SKILLS).** Run `scripts/capacity_analyzer.py` from this skill directory.
...
```bash
python3 scripts/capacity_analyzer.py \
