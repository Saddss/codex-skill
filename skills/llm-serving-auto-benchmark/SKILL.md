...
description: >-
  Framework-independent LLM serving benchmark for SGLang, vLLM, TensorRT-LLM.
  Config-driven search_space over launch flags under shared workload, GPU budget,
  and latency SLA. Use for cross-framework deployment comparison, cookbook sweeps,
  or finding best serve command before QPS binary search. Pairs with
  model-perf-binary-search for SLO-max-QPS tuning after a winner is chosen.
metadata:
  author: BBuf
  upstream: https://github.com/BBuf/AI-Infra-Auto-Driven-SKILLS/tree/main/skills/llm-serving-auto-benchmark
  adapted_for: Codex global skills (paths normalized to skill-relative)
---

> **Vendored from [BBuf/AI-Infra-Auto-Driven-SKILLS](https://github.com/BBuf/AI-Infra-Auto-Driven-SKILLS).** Run scripts from this skill directory (`cd` into `llm-serving-auto-benchmark/` or use paths relative to it). Workflow: this skill → `model-perf-binary-search` → `perf-analysis` / `llm-torch-profiler-analysis`.
...
```bash
python scripts/validate_cookbook_configs.py \
