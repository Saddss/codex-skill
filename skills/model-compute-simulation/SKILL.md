...
description: >-
  Build operator-level compute templates from model config; estimate FLOPs, MFU,
  tensor shapes, and TP/EP what-if for serving shapes. Use before benchmarking
  to sanity-check compute bounds or when writing REPORT.md theoretical sections.
metadata:
  author: BBuf
  upstream: https://github.com/BBuf/AI-Infra-Auto-Driven-SKILLS/tree/main/skills/model-compute-simulation
  adapted_for: Codex global skills (paths normalized to skill-relative)
---

> **Vendored from [BBuf/AI-Infra-Auto-Driven-SKILLS](https://github.com/BBuf/AI-Infra-Auto-Driven-SKILLS).** Run `scripts/model_compute_simulator.py` from this skill directory.
...
```bash
python3 scripts/model_compute_simulator.py "<model name>" --list-models
```
...
```bash
python3 scripts/model_compute_simulator.py "Qwen3-235B-A22B" \
...
python3 scripts/model_compute_simulator.py "Qwen3-235B-A22B" \
  ...
  --tp 8 --dp 1 --ep 8 \
  --gpu h20 --dtype bf16 \
  --kernel-flow @work/layer3_detail.json
```
...
```bash
python3 scripts/model_compute_simulator.py --list-gpus
```

Emit JSON for automation:

```bash
python3 scripts/model_compute_simulator.py "GLM-5" --format json
```
...
python3 scripts/extract_compute_flow_from_trace.py \
