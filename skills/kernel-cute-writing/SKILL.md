...
description: >
  Write GPU kernels using NVIDIA CuTe DSL (CUTLASS 4.x Python API) — NOT for
  Triton, CUDA C++, or conceptual-only questions. Covers element-wise kernels,
  GEMM patterns, reductions, memory hierarchy (global/shared/register/TMA),
  MMA tensor core ops, software pipelining, and framework integration.
  Use when implementing CUTLASS/CuTe kernels for inference GEMM, MoE, or
  attention in TRT-LLM / custom serving stacks.
license: Apache-2.0
metadata:
  author: NVIDIA Corporation
  upstream: https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/kernel-cute-writing
  adapted_for: Codex global skills (inference triggers added)
---

> **Vendored from [NVIDIA/TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM/tree/main/.claude/skills/kernel-cute-writing) under Apache-2.0.** Includes verify/benchmark scripts and extensive references. Pair with `perf-nsight-compute-analysis`. See `LICENSE-Apache-2.0.txt`.
